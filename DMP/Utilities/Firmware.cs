    ...
    public class Firmware
    {
        ...
        public Firmware()
        {
            ...
            string file = Path.GetDirectoryName(Application.ExecutablePath) + Path.DirectorySeparatorChar +
                          "FirmwareHistory.txt";
                          */
            string file = System.Reflection.Assembly.GetExecutingAssembly().Location + Path.DirectorySeparatorChar +
                          "FirmwareHistory.txt";
            if (!File.Exists(file))
            {
        ...
        {
            DMP.Dialogs.CustomMessageBox.Show("일단 주석처리. updateProgress" );
/*
            if (Progress != null)
...
*/
        ...
        public bool update(string comport, software temp, string historyhash)
        {
            ...
            try
            {
                ...
                {
                    DMP.Dialogs.CustomMessageBox.Show( Strings.CantDetectBoardVersion);
                    return false;
                }
                ...
                {
                    DMP.Dialogs.CustomMessageBox.Show(Strings.InvalidBoardType);
                    return false;
                }

                if (board < BoardDetect.boards.px4)
                {
                    DMP.Dialogs.CustomMessageBox.Show(Strings.ThisBoardHasBeenRetired, Strings.Note);
                }
            ...
            catch (Exception ex)
            {
                ...
                DMP.Dialogs.CustomMessageBox.Show("Failed to download new firmware : " + ex.ToString());
                return false;
            }
        ...
        public bool UploadPX4(string filename, BoardDetect.boards board)
        {
            ...
            {
                DMP.Dialogs.CustomMessageBox.Show(Strings.ErrorFirmwareFile + "\n\n" + ex.ToString(), Strings.ERROR);
                return false;
            }
            ...
            while (DateTime.Now < DEADLINE)
            {
                ...
                foreach (string port in allports)
                {
                    ...
                    {
                        log.Error(ex);
                        DMP.Dialogs.CustomMessageBox.Show("lost communication with the board.", "lost comms");
                        up.close();
                        return false;
                    }
                    catch
                    {
                        up.__reboot();
                        up.close();
                        DMP.Dialogs.CustomMessageBox.Show(Strings.NoNeedToUpload);
                        return true;
                    }
                    ...
                    DMP.Dialogs.CustomMessageBox.Show(Strings.PleaseWaitForTheMusicalTones);
            ...
            if (MainV2.comPort.BaseStream is SerialPort)
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        DMP.Dialogs.CustomMessageBox.Show(Strings.PleaseUnplugTheBoardAnd);
                    }
                }
                catch (Exception ex)
                {
        ...
        public bool UploadVRBRAIN(string filename, BoardDetect.boards board)
        {
            ...
            try
            {
                ...
                if (MainV2.comPort.getHeartBeat().Length > 0)
                {
                    ...
                    {
                        DMP.Dialogs.CustomMessageBox.Show(
...
//VR boards have no tone alarm
                        if (up.board_type == 1140)
                            DMP.Dialogs.CustomMessageBox.Show("Upload complete! Please unplug and reconnect board.");
                        else
                            DMP.Dialogs.CustomMessageBox.Show("Upload complete!");
                    }
                    else
                    {
                        // wait for IO firmware upgrade and boot to a mavlink state
...
/*
            ...
            if (pingReply == null || pingReply.Status != IPStatus.Success)
            {
                ...
                if (!ssidFound)
                {
                    DMP.Dialogs.CustomMessageBox.Show("Please connect to " + vehicleName + " Wifi now and after that press OK"
                        , vehicleName);
                ...
                while (pingReply == null || pingReply.Status != IPStatus.Success)
                {
                    if (!ssidFound)
                    {
                        if (DMP.Dialogs.CustomMessageBox.Show("You don't seem connected to " + vehicleName + " Wifi. Please connect to it and press OK to try again", vehicleName
                             ) == System.Windows.MessageBoxResult.Cancel )
                        {
                            return false;
                        }
                    }
                    else if (DMP.Dialogs.CustomMessageBox.Show("You seem connected to " + vehicleName + " Wifi but it didn't answer our request. Do you want to try again?", vehicleName
            ...
            try
            {
                ...
                if (!response.Contains("connected to 192.168.42.1:9050"))
                {
                    ...
                    DMP.Dialogs.CustomMessageBox.Show("Please press " + vehicleName + " Power button " + ntimes + " times", vehicleName
                        );//, MessageBoxButtons.OK);
                    response = adbClient.Connect(new DnsEndPoint("192.168.42.1", 9050));

                    while (!response.Contains("connected to 192.168.42.1:9050"))
                    {
                        if (DMP.Dialogs.CustomMessageBox.Show("Couldn't contact " + vehicleName + ". Press the Power button " + ntimes + " times. Do you want to try to connect again?", vehicleName
                ...
                DMP.Dialogs.CustomMessageBox.Show("Firmware installed!");
...
*/
        ...
        public bool UploadFlash(string comport, string filename, BoardDetect.boards board)
        {
                ...
                board == BoardDetect.boards.px4v3 || board == BoardDetect.boards.px4v4)
            {
                ...
                {
                    DMP.Dialogs.CustomMessageBox.Show("Please update, your install is currupt", Strings.ERROR);
                    return false;
                }
            }
        ...
        {
            DMP.Dialogs.CustomMessageBox.Show(" 이거 일단 주석 ");
        ...
        public bool UploadArduino(string comport, string filename, BoardDetect.boards board)
        {
            ...
            /*
                        ...
                        catch (Exception ex)
                        {
                            ...
                            DMP.Dialogs.CustomMessageBox.Show(Strings.FailedToReadHex + ex.Message);
                            return false;
                        }
                        ...
                        if (board == BoardDetect.boards.b1280)
                        {
                            ...
                            {
                                DMP.Dialogs.CustomMessageBox.Show("Firmware is to big for a 1280, Please upgrade your hardware!!");
                                return false;
                            }
                            //port = new ArduinoSTK();
                            port.BaudRate = 57600;
                        ...
                        try
                        {
                            ...
                            else
                            {
                                ...
                                DMP.Dialogs.CustomMessageBox.Show(Strings.CommunicationErrorNoConnection);
                            }
                            port.Close();
                        ...
                        {
                            updateProgress(0, Strings.FailedUpload);
                            DMP.Dialogs.CustomMessageBox.Show(Strings.CheckPortSettingsOr + ex);
                            try
                            {
                                port.Close();
                            }
        ...
        byte[] readIntelHEXv2(StreamReader sr)
        {
            ...
            while (!sr.EndOfStream)
            {
                ...
                if (line.StartsWith(":"))
                {
                    ...
                    {
                        DMP.Dialogs.CustomMessageBox.Show("The hex file loaded is invalid, please try again.");
            ...
            if (!hitend)
            {
                DMP.Dialogs.CustomMessageBox.Show("The hex file did no contain an end flag. aborting");
